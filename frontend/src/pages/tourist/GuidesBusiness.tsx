...
function BusinessesPreview({ load }: { load: () => Promise<{ data: { items?: any[] } }> }) {
  ...
  return <div className="mt-3 grid gap-3 md:grid-cols-3">{items.map((business) => <div className="rounded bg-white p-4 shadow" key={business.id}>{business.business_name || business.email}</div>)}</div>;
}
